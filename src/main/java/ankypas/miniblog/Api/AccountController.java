...
public class AccountController {
    ...
    public String register(@RequestBody HashMap<String, String> map){
        ...
        User user = new User(map.get("name"),map.get("sex"),Integer.parseInt(map.get("age")),map.get("profession"),map.get("personalProfile"),map.get("phone"),map.get("email"),map.get("password"));
        int statu = accountService.register(user);
        switch(statu){
            case 0:
                result.put("code",200);
                result.put("msg","注册成功！");
    ...
    public String login(@RequestBody HashMap<String, String> map){
        ...
        if(id > 0){
            ...
            result.put("code",200);
            result.put("msg","登录成功！");
            result.put("userOverview",accountService.getUserOverviewByUserId(id));
